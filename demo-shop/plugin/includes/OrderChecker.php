...
final class OrderChecker
{
    ...
    public function check(WC_Order $order, array $settings): array
    {
        ...
        if ($quote === null || $reference === null || $recipient === null) {
            ...
            // наш шлюз оплатить нельзя никогда — значит ему место в
            // on-hold с заметкой для продавца, а не в вечном pending.
            // Метка в мете здесь не подошла бы: исключить заказ из
            // выборки Scheduler такой меткой можно только через
            // meta_query, а он работает лишь на новом хранилище заказов
            // (HPOS) и молча отбрасывается на старом (в этом магазине оно
            // как раз старое) — метка ставилась бы, но выборку по-прежнему
            // занимали бы те же заказы. on-hold исчезает из выборки pending
            // сам, на любом хранилище, потому что запрос и так фильтрует
            // по статусу, а не по мете.
            $already_on_hold = $order->get_status() === 'on-hold';
            ...
            return ['status' => 'error', 'message' => 'Данные оплаты не найдены.', 'mutated' => !$already_on_hold];
        }
        ...
        try {
            try {
                ...
                return ['status' => 'unknown', 'message' => 'Не удалось проверить оплату. Пробуем ещё раз.', 'mutated' => false];
            }
            ...
            // вручную отменил заказ в админке, пока шла проверка. Честно:
            // единственная настоящая защита от гонки — лок (см. OrderLock),
            // а это перечитывание — вторая линия, и работает она не на
            // всех хранилищах заказов. wc_get_order() в пределах одного
            // запроса берёт заказ из объектного кеша, наполненного первым
            // чтением; на новом хранилище (HPOS) сброс кеша поста этот
            // кеш не затрагивает вовсе. В этом магазине хранилище старое
            // (записи заказов — обычные посты), и clean_post_cache()
            // реально заставляет следующий wc_get_order() перечитать
            // текущий статус из базы, а не из кеша первого чтения.
