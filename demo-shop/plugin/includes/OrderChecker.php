        ...
        try {
            ...
            if ($quote === null || $reference === null || $recipient === null) {
                ...
                if ($order->get_status() !== 'pending') {
                    return ['status' => 'error', 'message' => 'Данные оплаты повреждены, автоматическая проверка невозможна. Магазин свяжется с вами.', 'mutated' => false];
                }
                ...
                return ['status' => 'error', 'message' => 'Данные оплаты повреждены, автоматическая проверка невозможна. Магазин свяжется с вами.', 'mutated' => true];
            }

            // rpc_url берётся из текущих настроек шлюза, а сеть проверки —
            // из замороженной котировки заказа (иначе смена сети продавцом
            // задним числом проверяла бы старый заказ не там, где реально
            // был выпущен платёж). Если продавец переключил сеть в
            // настройках уже после создания заказа, rpc_url и cluster
            // заказа могут молча разойтись — это стоит записать в журнал,
            // а не оставить незамеченным.
