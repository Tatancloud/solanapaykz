    ...
    function check() {
        ...
        window.fetch(url, { credentials: 'same-origin' })
            ...
            .then(function (body) {
                ...
                // 'error' — тоже окончательный статус (например, заказ
                // провален из-за испорченных данных оплаты и больше не
                // будет автоматически проверяться): без него открытая
                // вкладка опрашивала бы сервер вечно, раз в 5 секунд.
                if (['paid', 'expired', 'mismatch', 'late', 'error'].indexOf(body.data.status) !== -1) {
                    stop();
