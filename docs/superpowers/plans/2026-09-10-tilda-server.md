...
- Отдаёт: `log.info(msg, fields?)`, `log.warn(...)`, `log.error(...)` —
  пишут в stdout строкой JSON, вырезая значения полей с именами
  `secret`, `password`, `pass`, `orderSecret`, `notifySecret`.
...
- секреты и пароль — непустые строки длиной не меньше 8 символов;
