...
- Отдаёт: `SolanaPayKzError`, `RateSourceError`, `RateUnavailableError`,
  ...
  `TOKENS`, `TokenSymbol`, `Cluster`, `resolveToken(cluster, token)`.
...
- [ ] **Шаг 6: Заменить node:crypto на изоморфный вызов**

`randomUUID` из `node:crypto` ломает работу в браузере. Заменить импорт на
глобальный `crypto`, доступный и в Node 18+, и в браузере:

```ts
// удалить строку: import { randomUUID } from 'node:crypto';
// и заменить вызов randomUUID() на:
const quoteId = crypto.randomUUID();
```

- [ ] **Шаг 7: Убедиться, что тесты по-прежнему проходят**

Запустить: `npx vitest run tests/quote.test.ts && npm run typecheck`
Ожидается: PASS и отсутствие ошибок типов.
...
import { resolveToken } from '../config.js';
import { convertKztToTokenUnits } from '../money.js';
import { isQuoteExpired, type Quote } from '../quote/quote.js';
...
export async function checkPayment(
  ...
  const { mint, decimals } = resolveToken(quote.cluster, quote.token);
  const expectedUnits = convertKztToTokenUnits(quote.amountKztCharged, quote.rate, decimals);
