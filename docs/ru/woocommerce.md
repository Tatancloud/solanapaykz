...
Готового архива в разделе релизов пока нет, поэтому два рабочих способа.

**Способ первый — собрать архив скриптом.** Он кладёт в архив только то, что
нужно в бою: без тестов, без зависимостей разработчика, без служебных файлов.

```bash
git clone https://github.com/Tatancloud/solanapaykz.git
cd solanapaykz/demo-shop/plugin
bash build.sh
# получится build/solanapaykz-0.1.0.zip — его и ставьте в WordPress
```

**Способ второй — скачать репозиторий целиком.** На странице
[репозитория](https://github.com/Tatancloud/solanapaykz) нажмите
«Code» → «Download ZIP», распакуйте и возьмите папку `demo-shop/plugin`.
Переименуйте её в `solanapaykz` и положите в `wp-content/plugins/`.

<div class="важно">
<strong>При втором способе в папке окажутся тесты и файлы разработки.</strong>
Работать плагин будет, но на боевом сайте лишним файлам не место: они
занимают место и расширяют поверхность атаки. Собранный архив из первого
способа чище.
</div>
