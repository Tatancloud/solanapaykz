...
<div class="важно">
<strong>Пакет пока не опубликован в npm.</strong> Команда ниже заработает
после публикации; до тех пор ставьте библиотеку прямо из репозитория — это
рабочий способ, а не временная заплатка: npm умеет ставить пакеты из git.
</div>
...
# сейчас — прямо из репозитория
npm install github:Tatancloud/solanapaykz
```

Установка из репозитория тянет исходники и собирает пакет на месте, поэтому
понадобится Node.js 22 и несколько секунд на сборку. Версия при этом
привязывается к текущему состоянию ветки `main`; если нужна
воспроизводимость, укажите коммит:
