    ...
    function check() {
        window.fetch(data.statusUrl, { credentials: 'same-origin' })
            ...
            .then(function (body) {
                ...
                if (body.state === 'ожидает' || body.state === 'не сошлось') {
                    // «Не сошлось» — не окончательное состояние (см.
                    // decision.ts, правка финального ревью): первой найденной
                    // транзакцией по метке платежа может быть протухший
                    // повтор кошелька или чужая транзакция, а не платёж
                    // покупателя, и сервер продолжает перепроверять заказ.
                    // Опрос из этой вкладки продолжается ровно так же, как
                    // при «ожидает» — иначе подтверждение, найденное сервером
                    // позже, покупатель увидел бы только вручную обновив
                    // страницу.
                    scheduleCheck(data.intervalMs);
                    return;
                }

                // Остальные состояния окончательны — сервер этот заказ
                // больше не тронет (см. decision.ts): опрашивать дальше
                // незачем, открытая вкладка иначе слала бы запросы раз в
                // 5 секунд вечно.
                stop();

                if (body.state === 'оплачен' || body.state === 'уведомлён' || body.state === 'просрочен') {
                    ...
                    // Если продавец настроил успех/отказ (задача 4
                    // финального ревью), обновлённая страница ответит
                    // редиректом на Tilda, а не своим итогом — тем же
                    // GET, которым отдавалась эта страница изначально.
