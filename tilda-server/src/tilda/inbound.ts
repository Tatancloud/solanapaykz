 ...
 * продублирован здесь, а не импортирован: SDK не выносит точность тенге в
 * публичный API (`src/index.ts` её не экспортирует), а менять экспорт
 * `@solanapaykz/core` — вне рамок этой задачи.
 */
