...
function проверитьСекрет(значение: unknown): значение is string {
  // Длина — в байтах UTF-8, а не в символах: кириллический пароль из 6 букв
  // (например, «пароль») даёт всего 6 code unit, но 12 байт — то есть по
  // энтропии он не короче латинской строки из 8+ символов. Считать в code
  // unit значило бы штрафовать нелатинские пароли без всякой причины.
  return typeof значение === 'string' && Buffer.byteLength(значение, 'utf8') >= МИНИМАЛЬНАЯ_ДЛИНА_СЕКРЕТА;
}
