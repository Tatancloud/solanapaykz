...
describe('сквозной путь через имитатор Tilda', () => {
  it('весь путь: заказ от Tilda, оплата, уведомление принято', async () => {
    ...
    // Тест финального ревью (задача 10): сквозной тест сверял подпись и
    // признак оплаты, но не саму сумму — а это единственное место во всём
    // тесте, где сумма проходит весь путь туда (заказ от Tilda) и обратно
    // (уведомление Tilda) через настоящий HTTP, а не только через прямой
    // вызов функции.
    expect(у.amount).toBe('15000');
