...
import {
  клиентскийАдрес,
  МАКСИМАЛЬНЫЙ_РАЗМЕР_КАРТЫ_ПОПЫТОК,
  создатьСчётчикПопытокВхода,
} from '../src/http/routes-admin.js';
...
describe('ограничение попыток входа считает по клиенту, а не всегда по сокету (правка ревью)', () => {
  ...
  it('семь запросов с СЕМЬЮ РАЗНЫМИ поддельными левыми адресами не обходят лимит — именно эта дыра была найдена ревью', async () => {
    // Тот же сценарий, которым ревью проверило дыру на живом сервере: nginx
    // ДОПИСЫВАЕТ настоящий адрес справа, но не удаляет то, что подставил
    // клиент, слева. Каждый из семи запросов ниже подставляет свой,
    // уникальный левый адрес — если бы код всё ещё читал левый элемент, все
    // семь считались бы разными клиентами и ни разу не получили бы 429.
    for (let i = 0; i < 7; i++) {
      const ответ = await запрос('POST', '/admin/login', {
        тело: { password: 'не тот' },
        xForwardedFor: `10.0.0.${i}, 9.9.9.9`,
      });
      if (i < 5) {
        expect(ответ.status).toBe(401);
      } else {
        // Шестой и седьмой запрос — уже сверх лимита для настоящего
        // адреса 9.9.9.9, несмотря на то что левая часть заголовка каждый
        // раз новая.
        expect(ответ.status).toBe(429);
      }
    }
...
describe('клиентскийАдрес (правка ревью — доверие X-Forwarded-For только от loopback)', () => {
  ...
  it('из цепочки в X-Forwarded-For берёт самый ПРАВЫЙ элемент — его дописал доверенный прокси, а не клиент', () => {
    // Правка финального ревью: nginx настроен на
    // proxy_add_x_forwarded_for — эта переменная СОХРАНЯЕТ то, что прислал
    // клиент, и ДОПИСЫВАЕТ настоящий адрес соединения СПРАВА. Значит
    // '203.0.113.7' здесь — то, что подставил сам клиент, а '198.51.100.1' —
    // то, что реально увидел nginx на TCP-соединении.
    expect(клиентскийАдрес('127.0.0.1', '203.0.113.7, 198.51.100.1')).toBe('198.51.100.1');
  });

  it('подделка левого элемента цепочки не меняет вычисленный адрес клиента — ловит именно ту дыру, что нашло ревью', () => {
    // Раньше код брал самый ЛЕВЫЙ элемент, то есть ровно то, что прислал
    // атакующий: семь запросов с семью разными поддельными заголовками
    // проходили как семь разных клиентов, и лимит попыток не срабатывал
    // ни разу (см. заголовок routes-admin.ts). Правый элемент — один и тот
    // же настоящий адрес — обязан давать один и тот же результат
    // независимо от того, что атакующий подставил слева.
    const левый1 = клиентскийАдрес('127.0.0.1', '1.1.1.1, 198.51.100.1');
    const левый2 = клиентскийАдрес('127.0.0.1', '2.2.2.2, 198.51.100.1');
    const безПодделки = клиентскийАдрес('127.0.0.1', '198.51.100.1');
    expect(левый1).toBe('198.51.100.1');
    expect(левый2).toBe('198.51.100.1');
    expect(левый1).toBe(левый2);
    expect(левый1).toBe(безПодделки);
  });
...
describe('создатьСчётчикПопытокВхода — карта не растёт без предела (правка финального ревью)', () => {
  it('запись, вышедшая из окна, удаляется из карты, когда добавляется новый адрес', () => {
    const счётчик = создатьСчётчикПопытокВхода();
    счётчик.отметитьНеудачу('1.1.1.1', 0);
    expect(счётчик.размер()).toBe(1);

    // 15 минут и 1 мс спустя запись для 1.1.1.1 уже вне своего окна —
    // добавление ВТОРОГО адреса должно вымести её, а не просто оставить
    // рядом навсегда.
    счётчик.отметитьНеудачу('2.2.2.2', 15 * 60 * 1000 + 1);
    expect(счётчик.размер()).toBe(1);
  });

  it('записи ВНУТРИ своего окна чистка не трогает', () => {
    const счётчик = создатьСчётчикПопытокВхода();
    счётчик.отметитьНеудачу('1.1.1.1', 0);
    счётчик.отметитьНеудачу('2.2.2.2', 1000);
    expect(счётчик.размер()).toBe(2);
    expect(счётчик.лимитИсчерпан('1.1.1.1', 1000)).toBe(false);
  });

  it('поток из ОГРОМНОГО числа разных адресов внутри одного окна не растит карту сверх потолка', () => {
    const счётчик = создатьСчётчикПопытокВхода();
    // Все — «прямо сейчас», ничего не успевает выйти из окна: единственная
    // защита здесь — жёсткий потолок, а не чистка устаревших.
    for (let i = 0; i < МАКСИМАЛЬНЫЙ_РАЗМЕР_КАРТЫ_ПОПЫТОК + 500; i++) {
      счётчик.отметитьНеудачу(`10.${Math.floor(i / 65536)}.${Math.floor(i / 256) % 256}.${i % 256}`, 0);
    }
    expect(счётчик.размер()).toBe(МАКСИМАЛЬНЫЙ_РАЗМЕР_КАРТЫ_ПОПЫТОК);
  });

  it('сброс попыток удаляет запись сразу, не дожидаясь истечения окна', () => {
    const счётчик = создатьСчётчикПопытокВхода();
    счётчик.отметитьНеудачу('1.1.1.1', 0);
