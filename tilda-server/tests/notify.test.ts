...
describe('notifyTilda', () => {
  ...
  it('повторяет с растущими паузами по расписанию 1/5/15/60 секунд', async () => {
    // Не через реальное ожидание (это честно ждало бы секунды) и не через
    // подмену задержкиMs на маленькие числа (это не доказало бы, что
    // расписание именно растущее) — через `подождать`: он получает те же
    // самые значения, что пошли бы в реальный `setTimeout`, но не ждёт их.
    ...
    expect(запрошенныеПаузы).toEqual([1000, 5000]);
  });
