...
describe('checkOrder', () => {
  ...
  it('расхождение сети в настройках и в заказе переводит заказ в «ошибка настроек», а не оставляет в «ожидает» навсегда', async () => {
    // Правка финального ревью: раньше заказ оставался «ожидает» без
    // ограничения по времени в listPending — переключение сети при выходе
    // в бой навсегда забивало бы очередь фонового обхода старыми тестовыми
    // заказами. Отдельное состояние выводит его из этой очереди сразу.
    const о = store.createOrder({ ...образец, cluster: 'mainnet' });
    const решение = await checkOrder(о, { ...deps, config: { ...config, cluster: 'devnet' } });
    expect(решение.action).toBe('ждать');
    expect(store.findByToken(о.token)?.state).toBe('ошибка настроек');
  ...
  it('расхождение адреса получателя в настройках и в заказе переводит заказ в «ошибка настроек»', async () => {
    ...
    // проверять чужим адресом, — блокчейн не трогаем, но и «ожидает» заказ
    // больше не остаётся (правка финального ревью, см. тест выше про сеть).
    const о = store.createOrder(образец);
  ...
  it('заказ в «ошибка настроек» больше не выбирается фоновым обходом (listPending)', async () => {
    const о = store.createOrder({ ...образец, cluster: 'mainnet' });
    await checkOrder(о, { ...deps, config: { ...config, cluster: 'devnet' } });
    const выборка = store.listPending(10, config.lateWindowSeconds, 86400, Math.floor(Date.now() / 1000));
    expect(выборка.map((з) => з.id)).not.toContain(о.id);
  });
  ...
  it('платёж после истечения цены переводит в «поздний» и тоже отправляет письмо продавцу', async () => {
    const письма = счётчикПисем();
    // Правка финального ревью (задача 3): «поздний» наступает не по
    // тому, что expiresAt в прошлом, а по тому, что заказ уже официально
    // отменён (state = «просрочен») — то есть предыдущая проверка уже не
    // нашла платёж и закрыла заказ ДО того, как эта транзакция была
    // замечена. Заказ, всё ещё «ожидает» на момент confirmed, теперь
    // завершается как «оплачен» независимо от истечения — см. соседний
    // тест decision.test.ts «платёж пришёл ровно на границе».
    const о = store.createOrder({ ...образец, expiresAt: Math.floor(Date.now() / 1000) - 10 });
    store.updateState(о.id, 'просрочен');
  ...
  it('платёж, обнаруженный, пока заказ ещё «ожидает», завершает его независимо от истечения цены', async () => {
    // Между отправкой платежа и попаданием транзакции в подтверждённый
    // блок проходит 15–30 секунд — покупатель мог заплатить точно в
    // срок, но сервер увидел это уже после истечения expiresAt. Раз
    // фоновый обход ещё не успел (или не должен был) закрыть заказ как
    // «просрочен» до этого момента, цена по нему всё ещё действовала.
    const письма = счётчикПисем();
    const о = store.createOrder({ ...образец, expiresAt: Math.floor(Date.now() / 1000) - 10 });

    await checkOrder(о, {
      ...deps,
      client: клиентСПлатежом('подпись-вовремя'),
      тест: { ...deps.тест, отправкаПисьма: письма.отправкаПисьма },
    });

    expect(store.findByToken(о.token)?.state).toBe('оплачен');
    await дождатьсяУсловия(() => письма.значение >= 1);
  });
  ...
  it('«не сошлось» — не тупик: заказ проверяется повторно, и второй проход с тем же расхождением не шлёт второе письмо', async () => {
    // Правка финального ревью (задача 2). Без неё второй вызов checkOrder
    // на заказе в состоянии «не сошлось» вообще не доходил бы до
    // decide() — белый список отсекал бы его раньше. Здесь же второй
    // вызов доходит и подтверждает то же самое расхождение, но письмо
    // продавцу не дублируется: сигнатура транзакции та же, что и в
    // первый раз, — это не новая находка.
    const письма = счётчикПисем();
    const клиентСНесовпадением: PaymentCheckerClient = {
      async checkPayment() {
        return { status: 'mismatch', signature: 'подпись-повтор', reason: 'сумма меньше' };
      },
    };
    const о = store.createOrder(образец);
    ...
    // Второй проход (например, следующий проход фонового обхода) видит
    // тот же заказ — белый список decide() его больше не отсекает.
    await checkOrder(store.findByToken(о.token)!, {
      ...deps,
      client: клиентСНесовпадением,
      тест: { ...deps.тест, отправкаПисьма: письма.отправкаПисьма },
    ...
    expect(store.findByToken(о.token)?.state).toBe('не сошлось');
    expect(письма.значение).toBe(1); // второго письма не было
  });

  it('«не сошлось» разрешается подтверждённым платежом на повторной проверке', async () => {
    // Первая проверка застаёт протухший повтор кошелька (или чужую
    // транзакцию по той же метке) — «не сошлось». Следующая проверка
    // видит уже настоящий платёж — заказ завершается как «оплачен», а
    // не остаётся заблокированным навсегда первым же расхождением.
    const о = store.createOrder(образец);
    let ответ: PaymentStatus = { status: 'mismatch', signature: 'подпись-старая', reason: 'сумма меньше' };
    const клиент: PaymentCheckerClient = { async checkPayment() { return ответ; } };

    await checkOrder(о, { ...deps, client: клиент });
    expect(store.findByToken(о.token)?.state).toBe('не сошлось');

    ответ = { status: 'confirmed', signature: 'подпись-настоящая', amountPaid: образец.amountToken };
    await checkOrder(store.findByToken(о.token)!, { ...deps, client: клиент });

    const после = store.findByToken(о.token);
    expect(после?.state).toBe('оплачен');
    expect(после?.txSignature).toBe('подпись-настоящая');
    await дождатьсяЗавершенияОтправки(store, о.token); // не оставляем фоновую отправку висеть после теста
  });
  ...
  describe('оплаченный, но ещё не уведомлённый заказ', () => {
    ...
    it('делает ровно одну попытку за вызов, а число попыток накапливается между вызовами', async () => {
      ...
      //
      // минимальныйИнтервалПовтораУведомленияSeconds: 0 — этот тест про
      // накопление счёта попыток, а не про минимальный интервал между
      // ними (см. отдельный тест ниже, правка финального ревью); без
      // этого второй вызов сразу за первым был бы законно придержан.
      ...
      await checkOrder(заказ, {
        ...deps,
        client: клиентОжидающий(),
        тест: { ...deps.тест, отправка: отправкаКотораяВсегдаПадает, минимальныйИнтервалПовтораУведомленияSeconds: 0 },
      });
    ...
    it('минимальный интервал между реальными попытками не даёт опросу слать их чаще (правка финального ревью)', async () => {
      // Ровно та находка ревью: опрос из вкладки покупателя раз в 5 секунд
      // без ограничения устраивал бы новую РЕАЛЬНУЮ попытку на КАЖДОМ
      // опросе. Второй вызов checkOrder сразу за первым (без выдуманного
      // хода времени, обычный тест) обязан быть придержан — по умолчанию,
      // без переопределения минимальныйИнтервалПовтораУведомленияSeconds.
      // Отправка всегда неудачна — иначе первая же попытка уведомила бы
      // Tilda успешно, и заказ вышел бы из ветки довоза уведомления вовсе
      // (см. проверитьЗаблокированный) не из-за троттлинга, а из-за успеха.
      const заказ = создатьОплаченныйНеуведомлённый(Math.floor(Date.now() / 1000) - 10);
      let вызововОтправки = 0;
      const отправкаКотораяВсегдаПадает: Отправка = async () => {
        вызововОтправки += 1;
      ...
      // Никакой ВТОРОЙ реальной попытки не было — интервал ещё не истёк.
      expect(вызововОтправки).toBe(1);
      expect(store.findByToken(заказ.token)?.notifyAttempts).toBe(1);
    });

    it('после истечения минимального интервала следующая попытка проходит', async () => {
      const заказ = создатьОплаченныйНеуведомлённый(Math.floor(Date.now() / 1000) - 10);
      let вызововОтправки = 0;
      const отправкаКотораяВсегдаПадает: Отправка = async () => {
        вызововОтправки += 1;
      ...
      // Интервал, укороченный тестовым крюком до нуля — эквивалент «время
      // прошло достаточно» без реального ожидания в прогоне теста.
      await checkOrder(store.findByToken(заказ.token)!, {
        ...deps,
        client: клиентОжидающий(),
        тест: {
          ...deps.тест,
          отправка: отправкаКотораяВсегдаПадает,
          минимальныйИнтервалПовтораУведомленияSeconds: 0,
        },
      });
      expect(вызововОтправки).toBe(2);
    });
