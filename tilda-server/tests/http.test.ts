...
describe('корневой адрес', () => {
  it('отдаёт страницу с объяснением, а не отказ «не найдено»', async () => {
    const ответ = await запрос('GET', '/');

    expect(ответ.status).toBe(200);
    // Обе версии на одной странице: посетитель не обязан угадывать язык.
    expect(ответ.body).toContain('payment server');
    expect(ответ.body).toContain('Деньги идут напрямую');
    // Предупреждение о приватных ключах обязано быть именно здесь: это
    // единственная страница, на которую попадают случайно, и первое, что
    // должен прочитать человек, подозревающий подвох.
    expect(ответ.body).toContain('мошенник');
  ...
  it('на корне не раскрывает ни настроек, ни секретов', async () => {
    ...
    expect(ответ.body).not.toContain('secret');
    expect(ответ.body.toLowerCase()).not.toContain('rpc');
  });
});
