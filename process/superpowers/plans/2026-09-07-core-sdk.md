...
**Спека:** `process/superpowers/specs/2026-09-07-core-sdk-design.md`
...
- Пустой или неположительный ответ источника = отказ источника, а не нулевой курс.
  Разделение ошибок: `RateSourceError` — источник ответил, но ответ непригоден
  (пустой объект, неположительное число, HTTP-ошибка, таймаут); `ConfigError` —
  синтаксически неверные данные на входе низкоуровневых функций. Фолбэк на
  резервный источник срабатывает при ЛЮБОЙ ошибке источника, тип роли не играет.
